#  checkmate + adaptive depth + evaluation_cycle
...
def score_evaluation_cycle(grid):
	height = len(grid)
	width = len(grid[0])
	score = 0.0
	n = 0
	...
	for i in range(4):
		score+=figureGrid[i][0] * (r ** n)
		n=n+1

	for i in range(1,4,1):
		score+=figureGrid[3][i] * (r ** n)
		n=n+1

	for i in range(2,-1,-1):
		score+=figureGrid[i][3] * (r ** n)
		n=n+1
	for i in range(2,0,-1):
		score+=figureGrid[0][i] * (r ** n)
		n=n+1
	for i in range(1,3,1):
		score+=figureGrid[i][1] * (r ** n)
		n=n+1
	for i in range(2,0,-1):
		score+=figureGrid[i][2] * (r ** n)
		n=n+1
	return score
...
def get_next_action(grid):
	...
	if len(get_empty_cells(grid)) >= 5:
		depth = 2
	elif len(get_empty_cells(grid)) < 5 and len(get_empty_cells(grid)) >= 3:
		depth = 4
	elif len(get_empty_cells(grid)) < 3:
		depth = 6
