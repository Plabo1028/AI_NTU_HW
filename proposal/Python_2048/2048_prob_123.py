...
def DFS(grid,depth):
	...
	if (num_empty_cells < 3):
		#return IM(grid, 0, depth*2)
		loop = 2 * (num_empty_cells+3) ** depth
	else:
	...
	for i in range(loop):
		...
		for j in range(depth):
			...
			if not prepare_next_turn(grid):
				max_score = max_score*j/depth/10.0
				#we can assign the depth to max_score, because larger depth should correspond to higher score even though the final score is 0 
				break
			max_score = 0
			best_move = ''
			for move in 'adws':
...
def main():
	...
	#grid = [[4, 16, 512, 1024], [64, 256, 128,32],[8,16,32,16],[4,8,2,2]]
	print_grid(grid)
	while True:
		grid_copy = copy.deepcopy(grid)
